/**
 * MasterPlan AI / Ton Society India — Universal Reality System (URS v1.0) Execution Engine
 ...
 * Gate 4: MasterPlan AI Tool Registration & PQC Binding
 * Gate 5: Pure-TS ML-DSA-65 Signing & Tamper Rejection
 * Gate 6: TON Smart Contract Gateway Post-Quantum Conjunction
 * Gate 7: NIST FIPS 203 ML-KEM-768 & §7.3 Implicit Rejection
 * Gate 8: TON FunC Smart Contract Source Conformance
...
import { TonPqcGateway } from '../src/ton_pqc_gateway.js';
...
console.log('╔══════════════════════════════════════════════════════════════════════════╗');
console.log('║   TON SOCIETY INDIA (MASTERPLAN AI) — UNIVERSAL REALITY SYSTEM (URS v1.0)║');
...
try {
  ...
  assert.strictEqual(manifest.system, 'TON-SOCIETY-INDIA-MASTERPLAN-AI');
...
// -----------------------------------------------------------------------------
try {
  const gateway = new TonPqcGateway();
  const { tool, hybridProof } = gateway.registerAiTool(
    'QMoosa Quantum Assistant',
    'quantum',
    'Freemium',
    'Intermediate'
  );
  assert.strictEqual(tool.verifiedByPqc, true);
  assert.ok(tool.contractAddress.startsWith('EQD'));
  const valid = gateway.verifyRegistration(tool.toolId, hybridProof);
  assert.strictEqual(valid, true, 'Registration proof must verify cryptographically');

  gates.push({
    gate: 4,
    name: 'MasterPlan AI Tool Registration & PQC Binding',
    passed: true,
    score: 1.0,
    details: 'Verified MasterPlan AI tool registration with cryptographic proof on TON address'
  });
  console.log('▶ [URS GATE 4/10] MasterPlan AI Tool Registration & PQC Binding');
  console.log('  ✅ Verified MasterPlan AI tool registration with cryptographic proof on TON address\n');
} catch (e: any) {
  gates.push({ gate: 4, name: 'MasterPlan AI Tool Registration & PQC Binding', passed: false, score: 0.0, details: e.message });
...
try {
  ...
  const sigResult = createPqcHybridSignature('TON_TOOL_PROOF_GATE5', keyPair, 0.05, 'ton-masterplan');
  assert.ok(sigResult.hybridSignature.startsWith('PQC-HYBRID-x402.'));

  const ver = verifyPqcSignature(sigResult.hybridSignature, 'TON_TOOL_PROOF_GATE5', keyPair.publicKey, 0.05, 'ton-masterplan');
  ...
  const verTampered = verifyPqcSignature(tamperedSig, 'TON_TOOL_PROOF_GATE5', keyPair.publicKey, 0.05, 'ton-masterplan');
...
try {
  const keyPair = generatePqcKeyPair('ML-DSA-65');
  const sigResult = createPqcHybridSignature('TON_GATEWAY_SETTLEMENT', keyPair, 0.1, 'ton-gateway');
  ...
  gates.push({
    gate: 6,
    name: 'TON Smart Contract Gateway Post-Quantum Conjunction',
    passed: true,
    score: 1.0,
    details: 'Dual hybrid post-quantum settlement verified with quantum resistance score 1.0'
  });
  console.log('▶ [URS GATE 6/10] TON Smart Contract Gateway Post-Quantum Conjunction');
  console.log('  ✅ Dual hybrid post-quantum settlement verified with quantum resistance score 1.0\n');
} catch (e: any) {
  gates.push({ gate: 6, name: 'TON Smart Contract Gateway Post-Quantum Conjunction', passed: false, score: 0.0, details: e.message });
...
try {
  ...
  const rawPair = ml_kem768.keygen(new Uint8Array(64).fill(0x6b));
...
// -----------------------------------------------------------------------------
try {
  const contractPath = path.resolve('contracts/masterplan_ai.fc');
  assert.ok(fs.existsSync(contractPath), 'contracts/masterplan_ai.fc must exist');
  const contractContent = fs.readFileSync(contractPath, 'utf8');
  assert.ok(contractContent.includes('recv_internal'), 'Must define TON recv_internal handler');
  assert.ok(contractContent.includes('load_data'), 'Must define contract storage loader');
  assert.ok(contractContent.includes('udict_set'), 'Must manage tools dictionary');

  gates.push({
    gate: 8,
    name: 'TON FunC Smart Contract Source Conformance',
    passed: true,
    score: 1.0,
    details: 'Verified TON FunC smart contract structure, internal receiver, and dictionary storage'
  });
  console.log('▶ [URS GATE 8/10] TON FunC Smart Contract Source Conformance');
  console.log('  ✅ Verified TON FunC smart contract structure, internal receiver, and dictionary storage\n');
} catch (e: any) {
  gates.push({ gate: 8, name: 'TON FunC Smart Contract Source Conformance', passed: false, score: 0.0, details: e.message });
